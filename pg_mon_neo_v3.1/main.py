...
class DatabaseViewer(QMainWindow):
    ...
    def log_message(self, category, message, level="INFO"):
        ...
        # Write to join log panel
        if category == "Join" and hasattr(self, 'join_log_text'):
            self.join_log_text.append(formatted_message)
    ...
    def init_ui(self):
        ...
        join_tab = QWidget()
        self.setup_join_tab_ui(join_tab)
        self.tab_widget.addTab(join_tab, "Join")
    ...
    def setup_join_tab_ui(self, parent):
        layout = QVBoxLayout()

        # Source, Join, and Target Label selection
        labels_layout = QHBoxLayout()
        
        source_layout = QVBoxLayout()
        source_layout.addWidget(QLabel("Source Label:"))
        self.join_source_label_combo = QComboBox()
        source_layout.addWidget(self.join_source_label_combo)
        
        join_layout = QVBoxLayout()
        join_layout.addWidget(QLabel("Join Label:"))
        self.join_label_combo = QComboBox()
        join_layout.addWidget(self.join_label_combo)
        
        target_layout = QVBoxLayout()
        target_layout.addWidget(QLabel("Target Label:"))
        self.join_target_label_combo = QComboBox()
        target_layout.addWidget(self.join_target_label_combo)
        
        labels_layout.addLayout(source_layout)
        labels_layout.addLayout(join_layout)
        labels_layout.addLayout(target_layout)
        layout.addLayout(labels_layout)
        ...
        # Source properties
        source_props_layout = QVBoxLayout()
        self.join_source_props_list = QListWidget()
        source_props_layout.addWidget(QLabel("Source Properties:"))
        source_props_layout.addWidget(self.join_source_props_list)
        self.join_source_props_table = QTableWidget()
        source_props_layout.addWidget(self.join_source_props_table)
        props_layout.addLayout(source_props_layout)
        
        # Join properties
        join_props_layout = QVBoxLayout()
        self.join_props_list = QListWidget()
        join_props_layout.addWidget(QLabel("Join Properties:"))
        join_props_layout.addWidget(self.join_props_list)
        self.join_props_table = QTableWidget()
        join_props_layout.addWidget(self.join_props_table)
        props_layout.addLayout(join_props_layout)
        
        # Target properties
        target_props_layout = QVBoxLayout()
        self.join_target_props_list = QListWidget()
        target_props_layout.addWidget(QLabel("Target Properties:"))
        target_props_layout.addWidget(self.join_target_props_list)
        self.join_target_props_table = QTableWidget()
        target_props_layout.addWidget(self.join_target_props_table)
        props_layout.addLayout(target_props_layout)
        
        layout.addLayout(props_layout)

        # Cypher Query Text Edit
        self.join_cypher_query_edit = QTextEdit()
        self.join_cypher_query_edit.setFixedHeight(100)
        self.join_cypher_highlighter = CypherHighlighter(self.join_cypher_query_edit.document())
        layout.addWidget(QLabel("Cypher Query:"))
        layout.addWidget(self.join_cypher_query_edit)
        ...
        # Relationship Name
        rel_layout = QHBoxLayout()
        self.join_relationship_name_combo = QComboBox()
        self.join_relationship_name_combo.setEditable(True)
        self.join_relationship_name_combo.addItems(self.get_relationship_types())
        self.join_relationship_name_combo.setCurrentText("JoinRelates")  # Default value

        rel_layout.addWidget(QLabel("Relationship Name:"))
        rel_layout.addWidget(self.join_relationship_name_combo)

        button_layout.addLayout(rel_layout)

        # Create Relationships button
        self.join_create_rel_button = QPushButton("Create Relationships")
        self.join_create_rel_button.clicked.connect(self.create_join_relationships)
        button_layout.addWidget(self.join_create_rel_button)

        # View Relationships button
        self.join_view_rel_button = QPushButton("View Relationships")
        self.join_view_rel_button.clicked.connect(self.view_join_relationships)
        button_layout.addWidget(self.join_view_rel_button)

        # View Graphically button
        self.join_view_graph_button = QPushButton("View Graphically")
        self.join_view_graph_button.clicked.connect(self.view_join_relationships_graphically)
        button_layout.addWidget(self.join_view_graph_button)

        layout.addLayout(button_layout)

        # Progress bar
        self.join_progress_bar = QProgressBar()
        layout.addWidget(self.join_progress_bar)


        # Log area
        self.join_log_text = QTextEdit()
        self.join_log_text.setReadOnly(True)
        self.join_log_text.setMaximumHeight(100)  # Set a maximum height
        layout.addWidget(QLabel("Log Messages:"))
        layout.addWidget(self.join_log_text)
        
        parent.setLayout(layout)

        # Connect signals
        self.connect_join_tab_signals()

        # Initial population of label combos
        self.populate_join_label_combos()

        # Initial update of Cypher query
        self.update_join_cypher_query()


    def limit_join_prop_selection(self):
        selected_items = self.join_props_list.selectedItems()
        if len(selected_items) > 2:
            for item in selected_items[2:]:
                item.setSelected(False)
        self.update_join_cypher_query()
        
    def connect_join_tab_signals(self):
        self.join_source_label_combo.currentTextChanged.connect(self.update_join_source_properties)
        self.join_label_combo.currentTextChanged.connect(self.update_join_properties)
        self.join_target_label_combo.currentTextChanged.connect(self.update_join_target_properties)
        self.join_source_props_list.itemSelectionChanged.connect(self.focus_join_source_property)

        # Allow multiple selection (up to 2) for join properties
        self.join_props_list.setSelectionMode(QAbstractItemView.SelectionMode.MultiSelection)
        self.join_props_list.itemSelectionChanged.connect(self.limit_join_prop_selection)

        self.join_target_props_list.itemSelectionChanged.connect(self.focus_join_target_property)
        self.join_source_label_combo.currentTextChanged.connect(self.update_join_cypher_query)
        self.join_label_combo.currentTextChanged.connect(self.update_join_cypher_query)
        self.join_target_label_combo.currentTextChanged.connect(self.update_join_cypher_query)
        self.join_source_props_list.itemSelectionChanged.connect(self.update_join_cypher_query)
        self.join_props_list.itemSelectionChanged.connect(self.update_join_cypher_query)
        self.join_target_props_list.itemSelectionChanged.connect(self.update_join_cypher_query)
        self.join_relationship_name_combo.currentTextChanged.connect(self.update_join_cypher_query)

    def populate_join_label_combos(self):
        if self.neo4j_driver is None:
            self.log_message("Neo4j", "Not connected to Neo4j. Please connect first.", "ERROR")
            return
        ...
        try:
            labels = self.get_neo4j_labels()
            self.join_source_label_combo.addItems(labels)
            self.join_label_combo.addItems(labels)
            self.join_target_label_combo.addItems(labels)
        except Exception as e:
            self.log_message("Neo4j", f"Error fetching labels: {str(e)}", "ERROR")

    def update_join_cypher_query(self):
        source_label = self.join_source_label_combo.currentText()
        join_label = self.join_label_combo.currentText()
        target_label = self.join_target_label_combo.currentText()
        relationship_name = self.join_relationship_name_combo.currentText()
        
        source_prop = self.join_source_props_list.currentItem().text() if self.join_source_props_list.currentItem() else None
        target_prop = self.join_target_props_list.currentItem().text() if self.join_target_props_list.currentItem() else None

        # Get selected join properties (up to 2)
        join_props = [item.text() for item in self.join_props_list.selectedItems()[:2]]

        if all([source_label, join_label, target_label, relationship_name, source_prop, target_prop]) and len(join_props) in [1, 2]:
            if len(join_props) == 1:
                query = f"""
    MATCH (source:`{source_label}`), (join:`{join_label}`), (target:`{target_label}`)
    WHERE source.`{source_prop}` = join.`{join_props[0]}`
    AND join.`{join_props[0]}` = target.`{target_prop}`
    CREATE (source)-[r1:`{relationship_name}`]->(target)
    RETURN count(r1) as rel_count
    """
            else:  # len(join_props) == 2
                query = f"""
    MATCH (source:`{source_label}`), (join:`{join_label}`), (target:`{target_label}`)
    WHERE source.`{source_prop}` = join.`{join_props[0]}`
    AND join.`{join_props[1]}` = target.`{target_prop}`
    CREATE (source)-[r1:`{relationship_name}`]->(target)
    RETURN count(r1) as rel_count
    """
            self.join_cypher_query_edit.setPlainText(query.strip())
        else:
            self.join_cypher_query_edit.setPlainText("Select all required fields to generate Cypher query.")


    def create_join_relationships(self):
        query = self.join_cypher_query_edit.toPlainText()

        if not query or query == "Select all required fields to generate Cypher query.":
            self.log_message("Join", "Please provide a valid Cypher query", "ERROR")
            return

        self.join_progress_bar.setValue(0)  # Reset progress bar
        ...
        try:
            with self.neo4j_driver.session() as session:
                ...
                self.log_message("Join", f"Created {rel_count} relationships", "INFO")
                
                # Refresh relationship types and update combo box
                relationship_types = self.get_relationship_types()
                current_rel = self.join_relationship_name_combo.currentText()
                self.join_relationship_name_combo.clear()
                self.join_relationship_name_combo.addItems(relationship_types)
                if current_rel in relationship_types:
                    self.join_relationship_name_combo.setCurrentText(current_rel)
                elif relationship_types:
                    self.join_relationship_name_combo.setCurrentIndex(0)
                
                # After creating relationships, automatically view them
                self.view_join_relationships()
        except Exception as e:
            self.log_message("Join", f"Error creating relationships: {str(e)}", "ERROR")
        finally:
            self.join_progress_bar.setValue(100)
            

    def view_join_relationships(self):
        relationship_name = self.join_relationship_name_combo.currentText()
        if not relationship_name:
            self.log_message("Join", "Please select a relationship name", "ERROR")
            return
        ...
        try:
            with self.neo4j_driver.session() as session:
                ...
                if not records:
                    self.log_message("Join", f"No relationships found for type: {relationship_name}", "INFO")
                else:
                    self.log_message("Join", f"Displaying relationships of type: {relationship_name}", "INFO")
                    for record in records:
                        source_props = ", ".join([f"{k}: {v}" for k, v in record['source_props'].items()])
                        target_props = ", ".join([f"{k}: {v}" for k, v in record['target_props'].items()])
                        relationship_info = f"({':'.join(record['source_labels'])} {{{source_props}}}) -[:{record['relationship_type']}]-> ({':'.join(record['target_labels'])} {{{target_props}}})"
                        self.log_message("Join", relationship_info, "INFO")
                
        except Exception as e:
            self.log_message("Join", f"Error viewing relationships: {str(e)}", "ERROR")


    def update_join_source_properties(self):
        label = self.join_source_label_combo.currentText()
        self.update_join_properties(label, self.join_source_props_list, self.join_source_props_table)

    def update_join_target_properties(self):
        label = self.join_target_label_combo.currentText()
        self.update_join_properties(label, self.join_target_props_list, self.join_target_props_table)
    


    def view_join_relationships_graphically(self):
        relationship_name = self.join_relationship_name_combo.currentText()
        if not relationship_name:
            self.log_message("Join", "Please select a relationship name", "ERROR")
            return
        ...
        def update_graph(limit):
            ...
            try:
                ...
                self.log_message("Join", f"Error viewing relationships graphically: {str(e)}", "ERROR")
                raise
    ...
    def update_join_properties(self, label=None, props_list=None, props_table=None, is_source=None):
        if label is None:
            label = self.join_label_combo.currentText()
        if props_list is None:
            props_list = self.join_props_list
        if props_table is None:
            props_table = self.join_props_table
        ...
        try:
            ...
            # Color matching properties
            self.color_matching_join_properties()
    ...
    def color_matching_join_properties(self):
        source_label = self.join_source_label_combo.currentText()
        join_label = self.join_label_combo.currentText()
        target_label = self.join_target_label_combo.currentText()

        source_props = set(self.join_source_props_list.item(i).text() for i in range(self.join_source_props_list.count()))
        join_props = set(self.join_props_list.item(i).text() for i in range(self.join_props_list.count()))
        target_props = set(self.join_target_props_list.item(i).text() for i in range(self.join_target_props_list.count()))
        
        source_join_common = source_props.intersection(join_props)
        join_target_common = join_props.intersection(target_props)
        
        color_map = {}
        for prop in source_join_common:
            color_map[prop] = self.generate_random_color()
        for prop in join_target_common:
            if prop not in color_map:
                color_map[prop] = self.generate_random_color()
        
        self.color_property_list(self.join_source_props_list, color_map)
        self.color_property_list(self.join_props_list, color_map)
        self.color_property_list(self.join_target_props_list, color_map)


    def focus_join_source_property(self):
        self.focus_property(self.join_source_props_list, self.join_source_props_table)

    def focus_join_property(self):
        self.focus_property(self.join_props_list, self.join_props_table)

    def focus_join_target_property(self):
        self.focus_property(self.join_target_props_list, self.join_target_props_table)

    def refresh_join_tab(self):
        self.log_message("Join", "Refreshing Join tab", "INFO")
        
        # Reload label names
        labels = self.get_neo4j_labels()
        relationship_types = self.get_relationship_types()
        
        # Update source label combo
        current_source = self.join_source_label_combo.currentText()
        self.join_source_label_combo.clear()
        self.join_source_label_combo.addItems(labels)
        if current_source in labels:
            self.join_source_label_combo.setCurrentText(current_source)
        elif labels:
            self.join_source_label_combo.setCurrentIndex(0)
        
        # Update join label combo
        current_join = self.join_label_combo.currentText()
        self.join_label_combo.clear()
        self.join_label_combo.addItems(labels)
        if current_join in labels:
            self.join_label_combo.setCurrentText(current_join)
        elif labels:
            self.join_label_combo.setCurrentIndex(0)
        
        # Update target label combo
        current_target = self.join_target_label_combo.currentText()
        self.join_target_label_combo.clear()
        self.join_target_label_combo.addItems(labels)
        if current_target in labels:
            self.join_target_label_combo.setCurrentText(current_target)
        elif labels:
            self.join_target_label_combo.setCurrentIndex(0)

        # Update relationship name combo
        current_rel = self.join_relationship_name_combo.currentText()
        self.join_relationship_name_combo.clear()
        self.join_relationship_name_combo.addItems(relationship_types)
        if current_rel in relationship_types:
        ...
        # Update properties and table contents
        self.update_join_source_properties()
        self.update_join_properties()
        self.update_join_target_properties()
        
        # Update Cypher query
        self.update_join_cypher_query()
        
        self.log_message("Join", "Join tab refreshed", "INFO")
